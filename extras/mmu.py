...
### CALIBRATION GCODE COMMANDS
    ...
    def cmd_MMU_CALIBRATE_GEAR(self, gcmd):
        ...
        reset = gcmd.get_int('RESET', 0, minval=0, maxval=1) # PAUL TODO add to doc
...
#######################
    ...
    def _handle_mmu_unsynced(self):
        ...
        if self.sync_feedback_operational:
            ...
            self._set_rotation_distance(self.rotation_distances[gate])
    ...
    def _update_sync_multiplier(self):
        ...
        self._set_rotation_distance(self.rotation_distances[self.gate_selected] / multiplier)
...
##############################################
    ...
    def _load_bowden(self, length):
        if length <= 0: return
        if self.calibrated_bowden_length > 0 and not self.calibrating: # PAUL self.calibrating?
...
#                self.log_info("Warning: Gate %d not calibrated! Using default 1.0 gear ratio!" % self.gate_selected)
        ...
        # "Fast" load
        _,_,_,delta = self._trace_filament_move("Fast loading move into bowden", length, track=True, encoder_dwell=reference_load) # PAUL dwell probably is False(?)
...
#################################
    ...
    def _trace_filament_move(self, trace_str, dist, speed=None, accel=None, motor="gear", homing_move=0, endstop_name="default", track=False, sync=False, wait=False, encoder_dwell=False):
        ...
        if motor in ["gear", "gear+extruder", "extruder"]:
            with self._wrap_sync_state(MmuToolHead.EXTRUDER_SYNCED_TO_GEAR if motor == "gear+extruder" else MmuToolHead.EXTRUDER_ONLY_ON_GEAR if motor == "extruder" else None):
                if homing_move != 0:
                    trig_pos = [0., 0., 0., 0.]
        ...
        # Extruder is driving, gear rail is following
            ...
            with self._wrap_sync_state(MmuToolHead.GEAR_SYNCED_TO_EXTRUDER):
                self._ensure_safe_extruder_temperature(wait=False)
                if homing_move != 0:
...
#################################
    ...
    @contextlib.contextmanager
    def _wrap_sync_state(self, sync_state=None):
        gear_synced = self.mmu_toolhead.is_gear_synced_to_extruder()
        self.mmu_toolhead.sync(sync_state)
        try:
            yield self
        finally:
            self.mmu_toolhead.sync(MmuToolHead.GEAR_SYNCED_TO_EXTRUDER) if gear_synced else self.mmu_toolhead.unsync()
...
############################
    ...
    def _set_gate_selected(self, gate):
        ...
        self._set_rotation_distance(self.rotation_distances[gate if gate >= 0 and self.variable_gate_ratios else 0])
        self._update_sync_multiplier()
        self.active_filament = {
