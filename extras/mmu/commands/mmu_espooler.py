...
class MmuEspoolerCommand(BaseCommand):
    ...
    def _run(self, gcmd):
        ...
        if self.check_if_no_espoolers(): return

        gate = gcmd.get_int('GATE', None, minval=0, maxval=mmu.num_gates - 1)
        if gate is not None and gcmd.get("UNIT", None) is not None:
            raise gcmd.error("UNIT parameter is not required if GATE is specified")
        mmu_unit = self.get_unit(gcmd, mode="infer", gate=gate)
        ...
        # Command is always available so check if mmu_unit can be operated on
        if mmu_unit.espooler is not None:
            if reset:
            ...
            if alloff:
                for gate in range(mmu.num_gates):
                    mmu_unit.espooler.set_operation(gate, 0, ESPOOLER_OFF)
                ...
                power = mmu_unit.p.espooler_assist_burst_power if loosen else mmu_unit.p.espooler_rewind_burst_power
                duration = mmu_unit.p.espooler_assist_burst_duration if loosen else mmu_unit.p.espooler_rewind_burst_duration
                ...
                # Determine power
                if burst:
                    default_power = mmu_unit.p.espooler_assist_burst_power if operation == ESPOOLER_ASSIST else mmu_unit.p.espooler_rewind_burst_power
                else:
                    default_power = mmu_unit.p.espooler_printing_power if operation == ESPOOLER_PRINT else 50
                power = gcmd.get_int('POWER', default_power, minval=0, maxval=100) if operation != ESPOOLER_OFF else 0

                if burst:
                    default_duration = mmu_unit.p.espooler_assist_burst_duration if operation == ESPOOLER_ASSIST else mmu_unit.p.espooler_rewind_burst_duration
                    ...
                    if mmu.is_printing():
                        ...
                        mmu_unit.espooler.set_operation(gate, power / 100, ESPOOLER_PRINT)

                elif operation != ESPOOLER_OFF:
                    mmu_unit.espooler.set_operation(gate, power / 100, operation)
                else:
                    mmu_unit.espooler.set_operation(gate, 0, ESPOOLER_OFF)

        if not quiet:
            msg = ""
            # Always give status on all gates (on all units)
            for gate in range(mmu.num_gates):
                u = mmu.mmu_unit(gate)
                if msg:
                    msg += "\n"
                msg += "{}".format(gate).ljust(2, UI_SPACE) + ": "
                if mmu.has_espooler(gate):
                    operation, value = u.espooler.get_operation(gate)
                    burst = ""
                    if operation == ESPOOLER_PRINT and value == 0:
                        burst = " [assist for %.1fs at %d%% power " % (u.p.espooler_assist_burst_duration, u.p.espooler_assist_burst_power)
                        if u.p.espooler_assist_burst_trigger:
                            burst += "on trigger, max %d bursts]" % u.p.espooler_assist_burst_trigger_max
                        else:
                            burst += "every %.1fmm of extruder movement]" % u.p.espooler_assist_extruder_move_length
