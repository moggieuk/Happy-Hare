...
class MmuSlicerToolMapCommand(BaseCommand):
    ...
    def _run(self, gcmd):
        ...
        initial_tool = gcmd.get_int('INITIAL_TOOL', None, minval=0, maxval=self.mmu.num_gates - 1)
        ...
        num_slicer_tools = gcmd.get_int('NUM_SLICER_TOOLS', self.mmu.num_gates, minval=1, maxval=self.mmu.num_gates)
        automap_strategy = gcmd.get('AUTOMAP', None)
        skip_automap = gcmd.get_int('SKIP_AUTOMAP', None, minval=0, maxval=1)

        quiet = False
        if reset:
            self.mmu._clear_slicer_tool_map()
            quiet = True
        else:
            # Ensure webhooks see a change if we edit map in place
            self.mmu.slicer_tool_map = dict(self.mmu.slicer_tool_map)

        # One-print option to suppress automatic automap
        if skip_automap is not None:
            self.mmu._restore_automap_option(bool(skip_automap))

        if tool >= 0:
            self.mmu.slicer_tool_map['tools'][str(tool)] = {'color': color, 'material': material, 'temp': temp, 'name': name, 'in_use': used}
            if used:
                self.mmu.slicer_tool_map['referenced_tools'] = sorted(set(self.mmu.slicer_tool_map['referenced_tools'] + [tool]))
                if not self.mmu.slicer_tool_map.get('skip_automap', False) and automap_strategy and automap_strategy != AUTOMAP_NONE:
                    self.mmu._automap_gate(tool, automap_strategy)
            if color:
                self.mmu._update_slicer_color_rgb()
            quiet = True

        if initial_tool is not None:
            self.mmu.slicer_tool_map['initial_tool'] = initial_tool
            self.mmu.slicer_tool_map['referenced_tools'] = sorted(set(self.mmu.slicer_tool_map['referenced_tools'] + [initial_tool]))
            quiet = True

        if total_toolchanges is not None:
            self.mmu.slicer_tool_map['total_toolchanges'] = total_toolchanges
            quiet = True
        ...
        if purge_volumes != "":
            try:
                ...
                self.mmu.slicer_tool_map['purge_volumes'] = [
                    [
        ...
        if not quiet:
            colors = sum(1 for t in self.mmu.slicer_tool_map['tools'] if self.mmu.slicer_tool_map['tools'][t]['in_use'])
            have_purge_map = len(self.mmu.slicer_tool_map.get('purge_volumes', [])) > 0
            msg = "No slicer tool map loaded"
            if colors > 0 or self.mmu.slicer_tool_map.get('initial_tool') is not None:
                ...
                for t, params in self.mmu.slicer_tool_map['tools'].items():
                    if params['in_use'] or detail:
                        msg += "T%d (gate %d, %s, %s, %d%sC)" % (int(t), self.mmu.ttg_map[int(t)], params['material'], params['color'], params['temp'], UI_DEGREE)
                        msg += " Not used\n" if detail and not params['in_use'] else "\n"
                if self.mmu.slicer_tool_map.get('initial_tool') is not None:
                    msg += "Initial Tool: T%d" % self.mmu.slicer_tool_map['initial_tool']
                    msg += " (will use bypass)\n" if colors <= 1 and self.mmu.tool_selected == TOOL_GATE_BYPASS else "\n"
                msg += "-------------------------------------------"
            if detail or purge_map or sparse_purge_map:
                if have_purge_map:
                    rt = self.mmu.slicer_tool_map['referenced_tools']
                    volumes = [row[:num_slicer_tools] for row in self.mmu.slicer_tool_map['purge_volumes'][:num_slicer_tools]]
