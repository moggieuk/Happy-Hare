...
class MmuTestCommand(BaseCommand):
    ...
    def __init__(self, mmu):
        ...
        self.register(
            name=self.CMD,
            handler=self.cmd_MMU_TEST,
            help_brief=self.cmd_MMU_TEST_help,
            help_params=self.cmd_MMU_TEST_param_help,
            help_supplement="",
    ...
    def cmd_MMU_TEST(self, gcmd):
        ...
        if gcmd.get_int('HELP', 0, minval=0, maxval=1):
            self.mmu.log_always(self.mmu.format_help(self.cmd_MMU_TEST_param_help), color=True)
            return
            ...
            if gcmd.get_int('SYNC_LOAD_TEST', 0, minval=0, maxval=1):
                ...
                for i in range(loop):
                    ...
                    if move_type in (0, 1):
                        ...
                        sync = "---" if self.mmu.mmu_toolhead.sync_mode is None else (
                            "E2G" if self.mmu.mmu_toolhead.sync_mode == DRIVE_EXTRUDER_SYNCED_TO_GEAR else
                            "G2E" if self.mmu.mmu_toolhead.sync_mode == DRIVE_GEAR_SYNCED_TO_EXTRUDER else "Ext"
                        )
                ...
                self.mmu.log_info(
                    ...
                    (total, self.mmu._get_filament_position())
                )
            ...
            if gcmd.get_int('SENSOR', 0, minval=0, maxval=1):
                ...
                if not loop:
                    ...
                    for pos_i in range(-1, 11):
                        self.mmu.log_always("check_all_sensors_before(%s,%s)=%s" % (pos_i, gate, self.mmu.sensor_manager.check_all_sensors_before(pos_i, gate, loading=loading)))
                    self.mmu.log_always("")
                    for pos_i in range(-1, 11):
                        self.mmu.log_always("check_all_sensors_after(%s,%s)=%s" % (pos_i, gate, self.mmu.sensor_manager.check_all_sensors_after(pos_i, gate, loading=loading)))
