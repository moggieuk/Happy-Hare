...
class MmuCheckGateCommand(BaseCommand):
                            ...
                            with mmu.wrap_suppress_visual_log():
                                ...
                                for gate, tool in gates_tools:
                                    ...
                                    if (gate in empty_gates
                                        ...
                                        mmu.gate_maps.set_gate_status(gate, GATE_EMPTY, clear_attributes=False)
                                    ...
                                    try:
                                        ...
                                        with mmu.wrap_suspend_insert_events():
                                            ...
                                            try:
                                                ...
                                                msg = "Gate %d marked EMPTY (no filament detected during homing)" % gate
                                                failures.append(msg)
                                                mmu.log_always(msg)
                                                continue
    ...
    def _check_path_unloaded(self, gate):
        ...
        if (mmu.filament_pos != FILAMENT_POS_UNLOADED
            ...
            raise MmuError("Cannot check gate %d: filament path is not safely unloaded" % gate)
        for endstop in SHARED_GATE_ENDSTOPS:
            if mmu._shared_gate_path_occupied(endstop, gate):
                raise MmuError("Cannot check gate %d: shared '%s' path is occupied" % (gate, endstop))
