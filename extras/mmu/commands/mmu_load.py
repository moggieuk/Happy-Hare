...
class MmuLoadCommand(BaseCommand):
    ...
    HELP_SUPPLEMENT = (
        "Examples:\n"
        + f"{CMD}                 ...Load filament from current gate\n"
        + f"{CMD} SKIP_PURGE=1    ...Load filament from current gate skipping purge macro\n"
        + f"{CMD} EXTRUDER_ONLY=1 ...Load filament into extruder (assumes filament is at extruder entrance)\n"
    )
