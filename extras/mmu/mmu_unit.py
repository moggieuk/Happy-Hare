...
class MmuUnit:
    ...
    def __init__(self, config, mmu_machine, unit_index, first_gate):
        ...
        selector_type = SELECTOR_LINEAR_SERVO
        ...
        has_bypass = 0              # Whether MMU design has bypass gate (also has to be calibrated on type-A designs)
        ...
        if self.mmu_vendor == VENDOR_ERCF:
            ...
            selector_type = SELECTOR_VIRTUAL
            variable_rotation_distances = 1
            variable_bowden_lengths = 0
            ...
            selector_type = SELECTOR_INDEXED
            variable_rotation_distances = 1
            variable_bowden_lengths = 0
            require_bowden_move = 1
            filament_always_gripped = 1
        ...
        if self.selector_type in ['LinearServoSelector', 'ServoSelector']:
        ...
        # Load optional mmu_encoder (can be a shared encoder)
        self.encoder = None
        ...
        # Load optional sync-feedback mmu_buffer (can be a shared buffer)
        self.buffer = None
...
class MmuToolHead(toolhead.ToolHead, object):
    ...
    def select_gear_stepper(self, gate):
        ...
        with self._resync_lock:
            if gate == 0:
                self._reconfigure_rail_no_lock([self.mmu_unit.gear_stepper])
            elif gate > 0:
                self._reconfigure_rail_no_lock(["%s_%d" % (self.mmu_unit.gear_stepper, gate)])
            else:
    ...
    def get_status(self, eventtime):
        ...
        res.update({
            'filament_pos': self.get_position()[1],
...
class MmuPrinterRail(_StepperPrinterRail, object):
    ...
    def lookup_endstop(self, endstop_pin, name, **kwargs):
        ...
        raise self.config.error("lookup_endstop() not supported on this Klipper version")
