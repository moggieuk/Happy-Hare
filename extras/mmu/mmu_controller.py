...
#bowden_fast_load_portion:   95
# And replace:
# gate_unload_buffer
# extruder_homing_buffer ??
...
#        ]
        ...
        # Logging and Stats
#PAUL        self.gcode.register_command('MMU_RESET', self.cmd_MMU_RESET, desc = self.cmd_MMU_RESET_help)
#PAUL        self.gcode.register_command('MMU_STATS', self.cmd_MMU_STATS, desc = self.cmd_MMU_STATS_help)
#PAUL        self.gcode.register_command('MMU_STATUS', self.cmd_MMU_STATUS, desc = self.cmd_MMU_STATUS_help)
...
#PAUL        self.gcode.register_command('MMU_HOME', self.cmd_MMU_HOME, desc = self.cmd_MMU_HOME_help)
#PAUL        self.gcode.register_command('MMU_SELECT', self.cmd_MMU_SELECT, desc = self.cmd_MMU_SELECT_help)
#PAUL        self.gcode.register_command('MMU_SELECT_BYPASS', self.cmd_MMU_SELECT_BYPASS, desc = self.cmd_MMU_SELECT_BYPASS_help) # Alias for MMU_SELECT BYPASS=1
#PAUL        self.gcode.register_command('MMU_PRELOAD', self.cmd_MMU_PRELOAD, desc = self.cmd_MMU_PRELOAD_help)
#PAUL        self.gcode.register_command('MMU_CHANGE_TOOL', self.cmd_MMU_CHANGE_TOOL, desc = self.cmd_MMU_CHANGE_TOOL_help)
...
# PAUL        self.gcode.register_command('MMU_TEST_CONFIG', self.cmd_MMU_TEST_CONFIG, desc = self.cmd_MMU_TEST_CONFIG_help)
    ...
    def register_commands(self):
        """
        Instantiate all mmu command classes in COMMAND_REGISTRY.
        """
        _commands = []

        for name, cls in sorted(COMMAND_REGISTRY.items()):
            try:
                _commands.append(cls(self))
            except Exception:
...
#
#        if config:
#            self.calibrated_bowden_length = self.mmu_unit().calibrator.get_bowden_length() # Temp scalar pulled from list for _f_calc()
#            msg += "\n\nLOAD SEQUENCE:"
#
#            # Gate loading
...
#######################
    ...
    def check_if_not_calibrated(self, required, silent=False, check_gates=None, use_autotune=True):
        ...
        # What mmu_units are involved with check_gates?
        mmu_units = { self.mmu_unit(g) for g in check_gates }
        ...
        for u in mmu_units:
            if not u.calibrator.check_calibrated(required):
                ...
                if (
                    ...
                    unit_check_gates = [gate for gate in check_gates if u.manages_gate(gate)]
                ...
                if (
                    ...
                    uncalibrated = u.calibrator.is_gear_rd_default(u.first_gate)
                    if uncalibrated:
                ...
                if (
                    ...
                    uncalibrated = [
                        ...
                        if u.calibrator.is_gear_rd_default(g) and gate in check_gates
                    ]
                    if uncalibrated:
                        if u.encoder:
                ...
                if (
                    ...
                    if u.variable_bowden_lengths:
                        uncalibrated = [
                            ...
                            if u.calibrator.is_bowden_length_default(g) and gate in check_gates
                        ]
# PAUL old logic. Why the first_gate+1?
#                        uncalibrated = [
#                            u.first_gate + i
#                            for i, value in enumerate(
#                                self.bowden_lengths[u.first_gate:u.first_gate + u.num_gates]
#                            )
#                            if value == -1 and (u.first_gate + i) in check_gates
#                        ]
                        if uncalibrated:
                        ...
                        uncalibrated = u.calibrator.is_bowden_length_default(u.first_gate)
                        if uncalibrated:
...
# PAUL does't make sense so removed!            homing_max += self.mmu_unit().p.gate_homing_max # Full bowden may not be quite enough
    ...
    def _load_bowden(self, length=None, start_pos=0.):
        ...
        try:
            ...
            if length > 0:
                ...
                if self.gate_selected > 0 and self.mmu_unit().is_gear_rd_default():
...
##############################################
    ...
    def load_sequence(self, bowden_move=None, skip_extruder=False, purge=None, extruder_only=False):
        ...
        bowden_length = self.mmu_unit().calibrator.get_bowden_length() # -1 if not calibrated
        ...
        try:
            ...
            if calibrating:
                self.mmu_unit().calibrator.update_bowden_calibration(calibrated_bowden_length)
            elif full and not extruder_only and not self.p.gcode_load_sequence:
                self.mmu_unit().calibrator.load_telemetry(bowden_move_ratio, homing_movement, deficit)
    ...
    def unload_sequence(self, bowden_move=None, check_state=False, form_tip=None, extruder_only=False):
        ...
        bowden_length = self.mmu_unit().calibrator.get_bowden_length() # -1 if not calibrated yet
        if bowden_length < 0:
...
#                    raise MmuError("Encoder sensed movement when the servo was released\nConcluding filament is stuck somewhere")
            ...
            if full and not extruder_only and not self.p.gcode_unload_sequence:
                self.mmu_unit().calibrator.note_unload_telemetry(bowden_move_ratio, homing_movement, deficit)
...
#            start_pos = self.mmu_toolhead.get_position()[1]
#            if motor in ["gear", "gear+extruder", "extruder"]:
#                with self._wrap_sync_mode(DRIVE_EXTRUDER_SYNCED_TO_GEAR if motor == "gear+extruder" else DRIVE_EXTRUDER_ONLY_ON_GEAR if motor == "extruder" else DRIVE_GEAR_ONLY):
#                    if homing_move != 0:
#                        trig_pos = [0., 0., 0., 0.]
#                        hmove = HomingMove(self.printer, endstops, self.mmu_toolhead())
#                        extruder_stepper = self.toolhead.get_extruder().extruder_stepper.stepper
...
#            # Extruder is driving, gear rail is following
#            elif motor in ["synced"]:
#                with self._wrap_sync_mode(DRIVE_GEAR_SYNCED_TO_EXTRUDER):
#                    if homing_move != 0:
#                        self.log_error("Not possible to perform homing move while synced")
#                    else:
#                        if self.log_enabled(LOG_STEPPER):
...
##
##        quiet = bool(gcmd.get_int('QUIET', 0, minval=0, maxval=1))
##        unit_index = gcmd.get_int('UNIT', self.unit_selected, minval=0, maxval=self.mmu_machine.num_units)
##
##        unit = self.mmu_unit(unit_index)
##        machine_params  = self.p   # MmuMachineParameters
...
##
##        # Determine unknown params (but exclude any params that are known in either set)
...
##
##        # Special-case calibration overrides (not ParamSpecs)
##        special_keys = set()
##
##        # 1) Clog length
##        key = 'mmu_calibration_clog_length'
##        special_keys.add(key)
##        if key in raw_keys_lc:
##            unknown = [n for n in unknown if n != key]
##
##            if unit.has_encoder():
##                length = gcmd.get_float('MMU_CALIBRATION_CLOG_LENGTH', minval=1., maxval=100.)
##                unit.calibrator.update_clog_detection_length(length, force=True)
##                applied = sorted(set(applied) | {key})
##            else:
##                raise gcmd.error("MMU_CALIBRATION_CLOG_LENGTH is only available when an encoder is present")
##
##        # 2) Bowden length
##        key = 'mmu_calibration_bowden_length'
##        special_keys.add(key)
##        if key in raw_keys_lc:
##            unknown = [n for n in unknown if n != key]
##            length = gcmd.get_float('MMU_CALIBRATION_BOWDEN_LENGTH', minval=0.)
##            unit.calibrator.update_bowden_length(self, length, console_msg=True)
##            applied = sorted(set(applied) | {key})
##
##        # Final unknown param detection
##        consumed_lc = set()
##        consumed_lc.update({'quiet', 'unit'})
##        consumed_lc.update({n.lower() for n in known})
##        consumed_lc.update(special_keys)
##
##        # raw keys that aren't consumable
##        truly_unknown = sorted(k for k in raw_keys_lc if k not in consumed_lc)
##
##        # Preserve your existing unknown/guarded behavior, but now with strict raw-args checking too
##        if truly_unknown:
##            raise gcmd.error("Unknown option(s): %s" % ", ".join(truly_unknown))
##
...
##
##        # Optional: report what changed (handy even if QUIET=1, but you can remove)
##        if applied and not quiet:
##            gcmd.respond_info("Applied: %s" % ", ".join(applied))
##
##        # List current params unless QUIET=1
...
##
##            msg.append("")
##            msg.append(f"\nMMU unit %s parameters ----------------" % unit.name)
...
##            if unit.variable_bowden_lengths:
##                msg.append("\nmmu_calibration_bowden_lengths = %s" % unit.calibrator.bowden_lengths)
##            else:
##                msg.append("\nmmu_calibration_bowden_length = %.1f" % unit.calibrator.bowden_lengths[0])
##            if unit.has_encoder():
##                msg.append("\nmmu_calibration_clog_length = %.1f" % unit.encoder.get_clog_detection_length())
##
##            gcmd.respond_info("\n".join(msg))
#
#    cmd_MMU_TEST_CONFIG_help = "Runtime adjustment of MMU configuration for testing or in-print tweaking purposes"
#    cmd_MMU_TEST_CONFIG_param_help = (
#        "MMU_TEST_CONFIG: %s\n" % cmd_MMU_TEST_CONFIG_help
...
#    )
#    cmd_MMU_TEST_CONFIG_supplement_help = (
#        "Examples:\n"
#        + "MMU_TEST_CONFIG extruder_homing_max=150 ...set the extruder_homing_max parameter to 150"
...
#
#        if gcmd.get_int('HELP', 0, minval=0, maxval=1):
#            self.mmu.log_always(self.format_help(self.cmd_MMU_TEST_CONFIG_param_help, self.cmd_MMU_TEST_CONFIG_supplement_help), color=True)
#            return
#
#        raw_params = gcmd.get_command_parameters()
#        raw_keys_lc = {k.lower() for k in raw_params.keys()}
...
#        # Report what changed
#        if applied and not quiet:
#            self.log_info("Applied parameters: %s" % ", ".join(applied))
#            self.log_info("(remember these are temporary changes until next restart)")
#
