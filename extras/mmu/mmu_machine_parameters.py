...
class MmuMachineParameters(TunableParametersBase):
    ...
    _ALL_SPECS: Sequence[ParamSpec] = (
        ...
        ParamSpec('gcode_load_sequence',           'int',       0, section="PRINTER"),
        ParamSpec('gcode_unload_sequence',         'int',       0, section="PRINTER"),
        ParamSpec('slicer_tip_park_pos',           'float',   0.0, section="PRINTER", limits=dict(minval=0.0)),
        ...
        ParamSpec('print_start_detection',         'int',       1, section="PRINTER", limits=dict(minval=0, maxval=1)),
        ParamSpec('startup_reset_ttg_map',         'int',       0, section="PRINTER", limits=dict(minval=0, maxval=1)),
        ...
        ParamSpec('action_changed_macro',          'str',   '_MMU_ACTION_CHANGED',     section="MACROS"),
        ParamSpec('print_state_changed_macro',     'str',   '_MMU_PRINT_STATE_CHANGED',section="MACROS"),
        ParamSpec('mmu_event_macro',               'str',   '_MMU_EVENT',              section="MACROS"),
        ...
        ParamSpec('unload_sequence_macro',         'str',   '_MMU_UNLOAD_SEQUENCE',    section="MACROS"),
        ParamSpec('load_sequence_macro',           'str',   '_MMU_LOAD_SEQUENCE',      section="MACROS"),
        ...
        ParamSpec('default_ttg_map',               'intlist', [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_status',           'intlist', [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_filament_name',    'list',    [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_material',         'list',    [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_color',            'list',    [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_temperature',      'intlist', [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_spool_id',         'intlist', [], section="DEFAULT_MAPS"),
        ParamSpec('default_gate_speed_override',   'intlist', [], section="DEFAULT_MAPS"),
        ParamSpec('default_endless_spool_groups',  'intlist', [], section="DEFAULT_MAPS"),

        ParamSpec('bypass_autoload',               'int',      1, section="BYPASS_ENCODER", limits=dict(minval=0, maxval=1)),
        ParamSpec('encoder_dwell',                 'float',  0.1, section="BYPASS_ENCODER", limits=dict(minval=0.0, maxval=2.0),  hidden=True),
        ParamSpec('encoder_move_step_size',        'float', 15.0, section="BYPASS_ENCODER", limits=dict(minval=5.0, maxval=25.0), hidden=True),

        # Configuration for extruder dimensions
        ParamSpec('toolhead_extruder_to_nozzle',   'float',  0.0, section="DIMENSIONS", limits=dict(minval=5.0)),
        ParamSpec('toolhead_sensor_to_nozzle',     'float',  0.0, section="DIMENSIONS", limits=dict(minval=1.0)),
        ParamSpec('toolhead_entry_to_extruder',    'float',  0.0, section="DIMENSIONS", limits=dict(minval=0.0)),
        ParamSpec('toolhead_residual_filament',    'float',  0.0, section="DIMENSIONS", limits=dict(minval=0.0, maxval=50.0)),
        ParamSpec('toolhead_ooze_reduction',       'float',  0.0, section="DIMENSIONS", limits=dict(minval=-5.0, maxval=20.0)),

        # TMC current control
        ParamSpec('extruder_form_tip_current',     'int',   100, section="TMC_CURRENT", limits=dict(minval=100, maxval=150)),
        ParamSpec('extruder_purge_current',        'int',   100, section="TMC_CURRENT", limits=dict(minval=100, maxval=150)),

        # Filament move speeds and accelaration
        ParamSpec('extruder_load_speed',           'float', 15.0, section="EXTRUDER_MOTION", limits=dict(minval=1.0)),
        ParamSpec('extruder_unload_speed',         'float', 15.0, section="EXTRUDER_MOTION", limits=dict(minval=1.0)),
        ParamSpec('extruder_sync_load_speed',      'float', 15.0, section="EXTRUDER_MOTION", limits=dict(minval=1.0)),
        ParamSpec('extruder_sync_unload_speed',    'float', 15.0, section="EXTRUDER_MOTION", limits=dict(minval=1.0)),
        ParamSpec('extruder_accel',                'float', 400.0,section="EXTRUDER_MOTION", limits=dict(above=10.0)),
        ParamSpec('extruder_homing_speed',         'float', 15.0, section="EXTRUDER_MOTION", limits=dict(minval=1.0)),

        # Macro toolhead tuning (max accel default is derived if 0)
        ParamSpec('macro_toolhead_max_accel',      'float',  0.0, section="MACRO_TOOLHEAD", limits=dict(minval=0.0)),
        ParamSpec('macro_toolhead_min_cruise_ratio','float', 0.0, section="MACRO_TOOLHEAD", limits=dict(minval=0.0, below=1.0)),

        # Optional features
        ParamSpec('spoolman_support',              'choice', SPOOLMAN_OFF,        section="FEATURES",
                  choices={o: o for o in SPOOLMAN_OPTIONS}),
        ParamSpec('t_macro_color',                 'choice', T_MACRO_COLOR_SLICER,section="FEATURES",
                  choices={o: o for o in T_MACRO_COLOR_OPTIONS}),
        ...
        ParamSpec('console_stat_columns',          'list', ['unload', 'load', 'total'], section="CONSOLE"),
        ParamSpec('console_stat_rows',             'list', ['total', 'job', 'job_average'], section="CONSOLE"),
        ParamSpec('console_gate_stat',             'choice', GATE_STATS_STRING, section="CONSOLE",
                  choices={o: o for o in GATE_STATS_TYPES}),
        ...
        ParamSpec('serious',                       'int', 0, section="CONSOLE", limits=dict(minval=0, maxval=1), hidden=True),
        ...
        ParamSpec('suppress_kalico_warning',       'int', 0, section="KLIPPER_TUNING", limits=dict(minval=0, maxval=1), hidden=True),
        ParamSpec('update_trsync',                 'int', 0, section="KLIPPER_TUNING", limits=dict(minval=0, maxval=1)),
        ParamSpec('canbus_comms_retries',          'int', 3, section="KLIPPER_TUNING", limits=dict(minval=1, maxval=10)),
        ParamSpec('update_bit_max_time',           'int', 0, section="KLIPPER_TUNING", limits=dict(minval=0, maxval=1)),
        ParamSpec('update_aht10_commands',         'int', 0, section="KLIPPER_TUNING", limits=dict(minval=0, maxval=1)),
    )
    ...
    def _post_load_fixups(self) -> None:
        # Normalize quotes exactly as original code did
