...
class MmuSensorManager:
    ...
    def reset_active_unit(self, unit):
        ...
        for sname, sensor in self.all_sensor_map.items():
            if unit == self.mmu.UNIT_UNKNOWN or (re.match(r'^unit\d+_', sname) and not name.startswith("unit%d_" % unit)):
        ...
        if unit != self.mmu.UNIT_UNKNOWN:
            for sname, sensor in self.unit_sensors[unit]:
                if name.startswith("unit%d_" % unit):
