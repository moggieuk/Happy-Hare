...
# -----------------------------------------------------------------------------------------------------------
    ...
    def gate_occupancy(self, gate):
        """
        What this gate's sensors say about it, ignoring the recorded gate_status:
    ...
    def _load_gate(self, allow_retry=True, extra_homing=0.0, mark_empty_on_failure=True):
        ...
        Args:
            ...
            mark_empty_on_failure: If True, a failed pickup marks the gate EMPTY, clears its
                attributes and sets filament position to UNLOADED. If False, attributes are
                kept and the gate's sensors decide: an untouched lane nothing can see raises
                MmuGateHomingMiss and is still EMPTY, anything else leaves position UNKNOWN.
    ...
    def _home_to_gate(self, profile, extra_homing=0.0, nfc=None, label="load", mark_empty_on_failure=True):
        ...
        if empty:
            self.gate_maps.set_gate_status(gate, GATE_EMPTY, clear_attributes=mark_empty_on_failure)
            self.set_filament_pos_state(FILAMENT_POS_UNLOADED)
