...
### CALIBRATION GCODE COMMANDS ###################################################
    ...
    def cmd_MMU_CALIBRATE_PSENSOR(self, gcmd):
        ...
        def _avg_raw(n=10, dwell_s=0.05):
            ...
            filter_raw = sensor.get_status(0).get('value_raw', None)

            for _ in range(int(max(1, n-1))):
                self.movequeues_dwell(dwell_s)
                filter_raw += k * (sensor.get_status(0).get('value_raw', None) - filter_raw) # 1st order low pass filter
                if filter_raw is None or not isinstance(filter_raw, float):
                    return None
            return (filter_raw)
            ...
            with self.wrap_sync_gear_to_extruder():
                with self.wrap_gear_current(percent=self.sync_gear_current, reason="while calibrating sync_feedback psensor"):
                    ...
                    for attempt in range (0, s_maxrange, 2):
                        ...
                        c_avg = _avg_raw()
                        if (c_avg >= 0.7 and c_avg > c_sd) or (c_avg <= 0.3 and c_avg < c_sd):
                            c_sd = c_avg 
                    ...
                    for attempt in range (0, s_maxrange, 2):
                        _,_,_,_ = self.trace_filament_move(msg, -2.0, motor="gear", speed=8, wait=True)
                        t_avg = _avg_raw()
                        if (t_avg >= 0.7 and t_avg > t_sd) or (t_avg <= 0.3 and t_avg < t_sd):
                          t_sd = t_avg 
