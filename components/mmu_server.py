...
class MmuParser:
    ...
    def gcode_processed_already(self, file_path):
    ...
    def parse_gcode_file(self, file_path):
        ...
        self.tools_used = sorted(tools_used)
        self.temps = temps
        self.materials = materials
        self.purge_volumes = purge_volumes
        self.filament_names = filament_names
        self.slicer = slicer
    ...
    def copy_tool_block(self, infile, outfile, last_toolchange_pos, next_toolchange, tool, x, y, e_max):
        ...
        if tool is None and e_max > 0 and insert_extrude_length:
            ...
            if self.insert_nextpos and x is not None and y is not None:
                toolchange += f' NEXT_POS="{x},{y}"'
            if self.insert_extrude_length:
                toolchange += f' EXTRUDE_LENGTH="{e_max}"'
            toolchange += f' ; T{tool}\n'
        ...
        while True:
            ...
            # Process placeholders before writing the line - but ONLY above the first extruding move
            if self.first_extrude_pos is None or cur_filepos < self.first_extrude_pos:
                line = add_placeholder(line)

            if line and cur_filepos < next_toolchange:
                outfile.write(line)
            else:
                break
    ...
    def process_file(self, input_filename, output_filename):
        ...
        with open(input_filename, 'r') as infile, open(output_filename, 'w') as outfile:
            ...
            while True:
                ...
                if (self.insert_extrude_length or self.first_extrude_pos is None) and line.startswith('G'):
                    ...
                    if e_match:
                        ...
                        if e_sum > e_max:
                            e_max = e_sum
                        if self.first_extrude_pos is None:
                            self.first_extrude_pos = cur_filepos

                # Check for the first toolhead motion in the tool block and record the target position
                if self.insert_nextpos and (x is None or y is None) and line.startswith('G'):
                    g1_match = g1_pattern.match(line)
                    if g1_match:
                        x, y = g1_match.groups()

                # Look for a MMU_START_LOAD_INITIAL_TOOL macro (essentially, a Tx with no number)
                # Note that we stop looking for one if we had any toolchanges or any extruding moves
                if tool is None and self.first_extrude_pos is None and line.startswith('MMU_START_LOAD_INITIAL_TOOL'):
                    copy_tool_block(infile, outfile, last_toolchange_pos, cur_filepos, tool, x, y, e_max)
    ...
    def add_placeholder(self, line):
        # Ignore comment lines to preserve slicer metadata comments
        if not line.startswith(";"):
            if METADATA_TOOL_DISCOVERY in line:
                if tools_used:
                    line = line.replace(METADATA_TOOL_DISCOVERY, ",".join(map(str, self.tools_used)))
                else:
                    line = line.replace(METADATA_TOOL_DISCOVERY, "0")
            if METADATA_COLORS in line:
                line = line.replace(METADATA_COLORS, ",".join(map(str, self.colors)))
            if METADATA_TEMPS in line:
                line = line.replace(METADATA_TEMPS, ",".join(map(str, self.temps)))
            if METADATA_MATERIALS in line:
                line = line.replace(METADATA_MATERIALS, ",".join(map(str, self.materials)))
            if METADATA_PURGE_VOLUMES in line:
                line = line.replace(METADATA_PURGE_VOLUMES, ",".join(map(str, self.purge_volumes)))
            if METADATA_FILAMENT_NAMES in line:
                line = line.replace(METADATA_FILAMENT_NAMES, ",".join(map(str, self.filament_names)))
        return line

    def main(self, path, filename, insert_placeholders=False, insert_nextpos=False, insert_extrude_length=False):
        file_path = os.path.join(path, filename)
        self.insert_nextpos = insert_nextpos
        self.insert_extrude_length = insert_extrude_length
        self.tools_used = self.colors = self.temps = self.materials = self.purge_volumes = self.filament_names = self.slicer = None
            ...
            if fname.endswith(".gcode") and not gcode_processed_already(file_path):
                with tempfile.TemporaryDirectory() as tmp_dir_name:
                    ...
                    if insert_placeholders:
                        start = time.time()
                        has_placeholder = parse_gcode_file(file_path)
                        metadata.logger.info("Reading placeholders took %.2fs. Detected gcode by slicer: %s" % (time.time() - start, self.slicer))
                    else:

                    if (self.insert_nextpos and self.tools_used is not None and len(tools_used) > 0) or self.insert_extrude_length or has_placeholder:
                        start = time.time()
                        msg = []
                        if has_placeholder:
                        ...
                        process_file(file_path, tmp_file)
...
if __name__ == "__main__":
    ...
    mmuParser.main(args.path, args.filename, args.placeholders, args.nextpos, True)
