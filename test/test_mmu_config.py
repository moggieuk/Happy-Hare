...
class TestLegacyBoolintMigration(unittest.TestCase):
    """Legacy BOOL + derived PARAM pairs migrate to their BOOLINT replacement."""

    KCONFIG = '''\
config PARAM_FEATURE_ON
  boolint "Feature on"
  default 0

config PARAM_FEATURE_OFF
  boolint "Feature off"
  default 1

config PARAM_SYNC_FEEDBACK_ENABLED
  boolint "Sync feedback"
  default 0

config PARAM_SYNC_TO_EXTRUDER
  boolint "Sync to extruder"
  default 0

config PARAM_SYNC_FORM_TIP
  boolint "Sync form tip"
  default 0
...
config VAR_BLOBIFIER_ENABLE_SHAKER
  boolint "Blobifier shaker"
  default 0

config PARAM_EXPLICIT
  boolint "Explicit"
  default 0

config PARAM_WITHOUT_DERIVED
  boolint "Without derived"
  default 0
'''
    ...
    def _load(self, contents):
        with open(self.config_path, 'w') as f:
            f.write(contents)
        kc = self.kconfiglib.Kconfig(self.kconfig_path, warn=False)
        kc.load_config(self.config_path)
        return kc

    def test_exact_names_restore_enabled_and_disabled_legacy_values(self):
        kc = self._load('''\
CONFIG_BOOL_FEATURE_ON=y
CONFIG_PARAM_FEATURE_ON=1 #~DEFAULT~#
# CONFIG_BOOL_FEATURE_OFF is not set
CONFIG_PARAM_FEATURE_OFF=0 #~DEFAULT~#
''')
        self.assertEqual(kc.syms['PARAM_FEATURE_ON'].str_value, '1')
        self.assertEqual(kc.syms['PARAM_FEATURE_OFF'].str_value, '0')
        self.assertEqual(kc.syms['PARAM_FEATURE_ON'].user_value, 2)
        self.assertEqual(kc.syms['PARAM_FEATURE_OFF'].user_value, 0)

        kc.write_config(self.config_path, save_old=False)
        with open(self.config_path) as f:
            saved = f.read()
        self.assertNotIn('BOOL_FEATURE_ON', saved)
        self.assertNotIn('BOOL_FEATURE_OFF', saved)
        self.assertIn('CONFIG_PARAM_FEATURE_ON=1\n', saved)
        self.assertIn('CONFIG_PARAM_FEATURE_OFF=0\n', saved)

    def test_nonmatching_param_and_var_aliases_are_migrated(self):
        kc = self._load('''\
CONFIG_BOOL_ENABLE_SYNC_FEEDBACK=y
CONFIG_PARAM_SYNC_FEEDBACK_ENABLED=1 #~DEFAULT~#
CONFIG_BOOL_ENABLE_SYNC_TO_EXTRUDER=y
CONFIG_PARAM_SYNC_TO_EXTRUDER=1 #~DEFAULT~#
CONFIG_BOOL_ENABLE_SYNC_FORM_TIP=y
CONFIG_PARAM_SYNC_FORM_TIP=1 #~DEFAULT~#
CONFIG_BOOL_ENABLE_SYNC_PURGE=y
CONFIG_PARAM_SYNC_PURGE=1 #~DEFAULT~#
CONFIG_BOOL_BLOBIFIER_ENABLE_SHAKER=y
CONFIG_VAR_BLOBIFIER_ENABLE_SHAKER=1 #~DEFAULT~#
''')
        self.assertEqual(kc.syms['PARAM_SYNC_FEEDBACK_ENABLED'].str_value, '1')
        self.assertEqual(kc.syms['PARAM_SYNC_TO_EXTRUDER'].str_value, '1')
        self.assertEqual(kc.syms['PARAM_SYNC_FORM_TIP'].str_value, '1')
        self.assertEqual(kc.syms['PARAM_SYNC_PURGE'].str_value, '1')
        self.assertEqual(kc.syms['VAR_BLOBIFIER_ENABLE_SHAKER'].str_value, '1')

    def test_stale_legacy_bool_does_not_override_explicit_boolint(self):
        kc = self._load('''\
CONFIG_BOOL_EXPLICIT=y
CONFIG_PARAM_EXPLICIT=0
''')
        self.assertEqual(kc.syms['PARAM_EXPLICIT'].str_value, '0')
        self.assertEqual(kc.syms['PARAM_EXPLICIT'].user_value, 0)

    def test_legacy_bool_requires_matching_derived_assignment(self):
        kc = self._load('CONFIG_BOOL_WITHOUT_DERIVED=y\n')
        self.assertEqual(kc.syms['PARAM_WITHOUT_DERIVED'].str_value, '0')
        self.assertIsNone(kc.syms['PARAM_WITHOUT_DERIVED'].user_value)
