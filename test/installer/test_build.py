...
class TestLedThemePrune(unittest.TestCase):
    ...
    it lists the installed shipped-theme files the unit no longer selects (the
    Makefile removes them). 'custom' and user-maintained theme files are never
    touched, and a unit without LEDs keeps no shipped theme files at all."""
    ...
    def _run(self, profile_name, overrides):
        ...
        try:
            ...
            with cfg._env(env), cfg._chdir(cfg.REPO_ROOT):
                pruned = build.prune_led_themes(tmpdir.name, [kcfg])
            return {os.path.basename(p) for p in pruned}
        finally:
            tmpdir.cleanup()
    ...
    def test_printer_without_a_theme_dir_is_a_noop(self):
        ...
        try:
            ...
            env = dict(cfg._SINGLE_UNIT_ENV, OUT=os.path.join(tmpdir.name, "out"))
            with cfg._env(env), cfg._chdir(cfg.REPO_ROOT):
                pruned = build.prune_led_themes(home, [kcfg])
