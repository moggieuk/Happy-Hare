...
def _save_state(selection, records):
    ...
    A two-file run must not forget the other twenty, and a `-k`-narrowed run of one file
    must not replace that file's whole-file time with its own smaller one.
    """
    _, previous = _load_state()
    for module, (seconds, count) in records.items():
        if count >= previous.get(module, (0.0, 0))[1]:
...
def _run(sink, argv, selection, entries):
    ...
    _save_state(list(selection) if narrowed else _load_state()[0], measured)
