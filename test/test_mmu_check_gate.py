...
class TestCheckGateSafety(EndlessSpoolTestCase):
    ...
    def assert_metadata_cleared(self):
        """Contrast assert_metadata_preserved, for gates never concluded empty."""
        self.assertEqual(self.hh.mmu.gate_material[0], '')
        self.assertEqual(self.hh.mmu.gate_color[0], '')
        self.assertEqual(self.hh.mmu.gate_spool_id[0], -1)
        self.assertEqual(self.hh.mmu.gate_temperature[0],
                         int(self.hh.mmu.gate_maps.p.default_extruder_temp))
    ...
    def test_confirmed_empty_gate_is_not_moved_and_is_cleared(self):
        self.remove(0)
        self.fil.history.clear()
        self.hh.run_gcode('MMU_CHECK_GATE TOOLS=0')
    ...
    def test_empty_gate_unassigns_its_spool_in_spoolman(self):
        # Must push the un-assign, not re-assert the spool on a gate just confirmed empty.
        self.hh.run_gcode('MMU_TEST_CONFIG spoolman_support=push')
        self.remove(0)
        pushes = []
        with patch.object(self.hh.mmu, '_spoolman_push_gate_map',
                          side_effect=lambda gate_ids: pushes.append(list(gate_ids))):
            self.hh.run_gcode('MMU_CHECK_GATE TOOLS=0')
        self.assertIn([(0, -1)], pushes)
        self.assert_metadata_cleared()

    def test_in_print_pause_keeps_the_required_tool_wording(self):
        # The pre-existing message users and their pause handlers match on.
        self.remove(0)
        self.hh.run_gcode('MMU_TEST_CONFIG endless_spool_on_load=0')
        self.hh.run_gcode('MMU_CHECK_GATE TOOLS=0')
        self.assertTrue(any('Required Tool T0 on gate 0 marked EMPTY' in e
                            for e in self.hh.errors), self.hh.errors)

    def test_outside_a_print_there_is_no_required_prefix(self):
        self.hh.run_gcode('MMU_TEST_CONFIG test_force_in_print=0 endless_spool_on_load=0')
        self.remove(0)
        self.hh.run_gcode('MMU_CHECK_GATE TOOLS=0')
        self.assertEqual(self.hh.errors, [])
        self.assertTrue(any('marked EMPTY' in line for line in self.hh.console))
        self.assertFalse(any('Required' in line for line in self.hh.console))

    def test_refusal_names_a_remedy(self):
        # A refusal on the command users reach for when recovering must say what to run.
        self.hh.place_filament(1, position=self.fil.layout['mmu_shared_exit'] + 5.)
        self.hh.run_gcode('MMU_CHECK_GATE TOOLS=0')
        self.assertTrue(any('MMU_RECOVER' in e and 'MMU_UNLOAD' in e
                            for e in self.hh.errors), self.hh.errors)

    def test_suppressed_recovery_is_explained(self):
        from extras.mmu.mmu_utils import MmuError
        with patch.object(self.hh.mmu, '_unload_gate', side_effect=MmuError('parking failure')):
            self.hh.run_gcode('MMU_CHECK_GATE TOOLS=0')
        self.assertTrue(any('position is unknown' in line for line in self.hh.console),
                        [l for l in self.hh.console if 'recover' in l.lower()])
