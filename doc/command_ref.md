...
```
  ...
  | `MMU_LED` | Quick way to try/test modes of operation of optional MMU LEDs  | `ENABLE=[0\|1]` Whether LED's are operational or not <br> `EFFECT=[off\|gate_status\|filament_color\|custom_color]` Selects the default effect for gate LEDs when no action is taking place <br> `EXIT_EFFECT=[off\|filament_color\|custom_color]` Selects the default effect for exit LED when no action is taking place <br> `GATE=.. COLOR=[w3c_color_string\|RRGGBB]` this combo of options is used to set the "custom" gate color map (shown with `custom_color` effect option) |
  ...
  | `MMU_SLICER_TOOL_MAP` | Used to set or display (no parameters) tools used in print. Typically this will be set in the print_start macro based on placeholders from the slicer including filament color, material type and temperature setting. This detail can be used in print by reading the printer variable. For example, `printer.mmu.slicer_tool_map.T5.material` will contain the material the slicer is expecting in tool 5 | `TOOL` the tool to set <br> `MATERIAL` the material type e.g. PLA or ABS <br> `COLOR` the color in form RRBBGG <br> `TEMP` the filament print temperature <br> `PURGE_VOLUMES` a comma separated list for the NxN purge volumes when changing tools <br> `QUIET=[0\|1]` whether to suppress output after set <br> `RESET=[0\|1]` used to reset/clear the map <br> `DETAIL=1` will dump the purge volumes map |
  <br>
