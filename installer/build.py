...
def prune_led_themes(config_home, kcfgs):
    """List the installed LED theme files no unit selects anymore (install-time
    ...
    keeps no shipped theme files at all. Returns the paths to remove; the
    Makefile removes them with $(SUDO)rm -f because the builder runs
    unprivileged while a system install's config tree is root-owned."""
    src = os.getenv("SRC", ".")
    ...
    for kcfg in kcfgs:
        ...
        for name in shipped:
            ...
            if os.path.lexists(path):
                ...
                logging.info("Pruning unselected LED theme '%s'" % path)
    return to_prune
...
def main():
    ...
    if args.prune_led_themes:
        for path in prune_led_themes(args.prune_led_themes[0],
